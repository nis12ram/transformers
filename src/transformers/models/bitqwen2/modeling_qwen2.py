...
from ..bitnet import BitLinear, BitLinearInf
...
class Qwen2MLPInf(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.gate_proj = BitLinearInf(
            self.hidden_size, self.intermediate_size, bias=False
        )
        self.up_proj = BitLinearInf(
            self.hidden_size, self.intermediate_size, bias=False
        )
        self.down_proj = BitLinearInf(
...
class Qwen2AttentionInf(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """
    ...
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        self.q_proj = BitLinearInf(
            self.hidden_size, self.num_heads * self.head_dim, bias=True
        )
        self.k_proj = BitLinearInf(
            self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True
        )
        self.v_proj = BitLinearInf(
            self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True
        )
        self.o_proj = BitLinearInf(
            self.num_heads * self.head_dim, self.hidden_size, bias=False
        )
...
class Qwen2SdpaAttentionInf(Qwen2AttentionInf):
    """
    Qwen2 attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `Qwen2Attention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
...
QWEN2Inf_ATTENTION_CLASSES = {
    "eager": Qwen2AttentionInf,
    "flash_attention_2": Qwen2FlashAttention2,
    "sdpa": Qwen2SdpaAttentionInf,
}

class Qwen2DecoderLayerInf(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.self_attn = QWEN2Inf_ATTENTION_CLASSES[config._attn_implementation](
            config, layer_idx
        )
        # self.self_attn = Qwen2AttentionInf(
        #     config, layer_idx
        # )

        self.mlp = Qwen2MLPInf(config)
        self.input_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Qwen2RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class Qwen2ModelInf(Qwen2PreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Qwen2DecoderLayer`]
    ...
    def __init__(self, config: Qwen2Config):
        ...
        self.layers = nn.ModuleList(
            [
                Qwen2DecoderLayerInf(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self._attn_implementation = config._attn_implementation
...
class BitQwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
...
class BitQwen2ForCausalLMInf(Qwen2PreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen2ModelInf(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
